...
function get20s(arr){
  ...
  arr.forEach(artist => {
    ...
    if (dates[0] > 1899 && dates[0] < 2001 && dates[2] > 1899 && dates[2] < 2001)
    {
      if (!deadArr.includes(artist.name)){
      deadArr.push(artist.name);
    }
    }
...
function lotsOfArt(arr){
  ...
  arr.forEach(artist => {
    ...
    {
      if(!beastArr.includes(artist.name))
      {
        beastArr.push(artist.name);
      }
    }
  });
  return beastArr;
